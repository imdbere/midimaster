...
function CopyBtn(props: { text: string }) {
  ...
  function copy() {
    ...
    writeClipboard(props.text).then(finish).catch(() => {
      // execCommand fallback for HTTP contexts (e.g. Android on local network)
      ...
      try { if (document.execCommand('copy')) finish() } catch { }
